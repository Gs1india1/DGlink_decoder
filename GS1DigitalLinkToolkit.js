class GS1DigitalLinkToolkit {
	...
	constructor() {
		...
		var tableP = {"00":2,"01":2,"02":2,"10":2,"11":2,"12":2,"13":2,"15":2,"16":2,"17":2,"20":2,"21":2,"22":2,"23":3,"24":3,"25":3,"30":2,"31":4,"32":4,"33":4,"34":4,"35":4,"36":4,"37":2,"39":4,"40":3,"41":3,"42":3,"70":4,"71":3,"80":4,"81":4,"82":4,"90":2,"91":2,"92":2,"93":2,"94":2,"95":2,"96":2,"97":2,"98":2,"99":2};
...
//		console.log("uriPathInfo="+uriPathInfo);
	...
	compressGS1DigitalLink(gs1DigitalLinkURI,useShortText) {
		var cursor=0;	
		var queryString="";
		var uriPathInfo="";
		var uriStem="";
		...
		// strip off https:// or http:// 
		if (gs1DigitalLinkURI.indexOf("http://") == 0) { cursor=7; protocol="http://" }
		if (gs1DigitalLinkURI.indexOf("https://") == 0) { cursor=8; protocol="https://" }

		var firstSlash = gs1DigitalLinkURI.substr(cursor).indexOf("/");
		uriStem=protocol+gs1DigitalLinkURI.substr(cursor).substr(0,firstSlash);
		var obj=this.extractFromGS1digitalLink(gs1DigitalLinkURI);
		
		var compressedLink = this.buildCompressedGS1digitalLink(obj,useShortText,uriStem);
		return compressedLink;
	}
	
	decompressGS1DigitalLink(gs1DigitalLinkURI,useShortText) {
		var cursor=0;	
		var queryString="";
		var uriPathInfo="";
		var candidates={};
		...
		var firstSlash = gs1DigitalLinkURI.substr(cursor).indexOf("/");
		var uriStem=protocol+gs1DigitalLinkURI.substr(cursor).substr(0,firstSlash);

		var obj=this.extractFromCompressedGS1digitalLink(gs1DigitalLinkURI)
		return this.buildGS1digitalLink(obj,useShortText,uriStem)
	}
...
//		while ((totallength-cursor) >=6)  {
		...
		while ((totallength-cursor) >8)  {
			...
			if (this.tableP.hasOwnProperty(h1h2)) {
				...
				if (this.tableF.hasOwnProperty(ai)) {
					...
					// handle fixed-length alphanumeric component, character count=C3
					// to support new AI 723s (certificate reference)
					if ((tx.hasOwnProperty('C3')) && (tx.C3 > 0)) {
						var encBits=binstr.substr(cursor,2);
						cursor+=2;				
					
						var enc=parseInt(encBits,2);
						if (enc==0) {
						// digits only at 3.32 bits per character
							var b1=Math.ceil(tx.C3*Math.log(10)/Math.log(2));
							var rbv=binstr.substr(cursor,b1);
							cursor+=b1;

							var s=parseInt(rbv,2).toString();
							s=this.padToLength(s,tx.C3);
							gs1AIarray[ai]=s;
						
						}

						if (enc==1) {
						// 5 bits per character A-Z - TODO - not yet implemented
				...
				throw new Error("Fail: we have not yet defined how to handle h1h2 when outside the range 00-99");
				// else branch 1
	...
	compressGS1AIarrayToBinary(gs1AIarray) {
		var binstr="";
		var cursor;
		var value;
		for (var key in gs1AIarray) {
			if (gs1AIarray.hasOwnProperty(key)) {
				var tx=this.tableF[key];
				cursor=0;
				value=gs1AIarray[key];
			
				// construct binary encoding of AI key as sequence of 4-bit digits
				var binAI="";
				for (var i=0; i<key.length; i++) {
					var digit=key.charAt(i);
					var binDigit=parseInt(digit).toString(2);
...
//					console.log("binValue="+binValue);
				...
				if ((tx.hasOwnProperty("C4")) && (tx.C4 > 0)) {
					...
					// determine encoding
					// use '00' if all-numeric
					// use '01' if 5-bit - alphabet to be defined - not yet implemented
					// use '10' if 6-bit within URI safe base 64 alphabet
					// use '11' if 7-bit ASCII
				
					var encoding='11';
					var regexAllNum=new RegExp("^[0-9]+$");
					var regexSafe64=new RegExp("^[A-Za-z0-9_-]+$");
					if (regexSafe64.test(charstr)) { encoding='10'; }
					if (regexAllNum.test(charstr)) { encoding='00'; }
...
//						console.log("binValue="+binValue);
	...
	buildCompressedGS1digitalLink(gs1AIarray,useShortText,uriStem) {
		...
		path="/"+this.bin2base64(this.compressGS1AIarrayToBinary(gs1AIarray));
	...
	gs1ElementStringsToCompressedGS1DigitalLink(elementString,useShortText,uriStem,uncompressedPrimary) {
		if (uncompressedPrimary) {
			...
			return this.buildGS1digitalLink(separated.ID,useShortText,uriStem)+"/"+this.bin2base64(this.compressGS1AIarrayToBinary(separated.nonID));
					
		} else {
			return this.buildCompressedGS1digitalLink(this.extractFromGS1elementStrings(elementString),useShortText,uriStem);
		}
	}
